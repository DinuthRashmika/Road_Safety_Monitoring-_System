...
class PlateOwnerService:
    def __init__(self):
        ...
        self.violation_model = None  # <--- New Model
    ...
    def perform_ocr(self, plate_region):
        ...
        for strategy in strategies:
            ...
            try:
                results = self.ocr_reader.readtext(img, decoder='greedy', allowlist='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', batch_size=1, detail=1, paragraph=False, mag_ratio=3.0, text_threshold=0.6, link_threshold=0.4, canvas_size=1280)
        ...
        if best_text: return [{'text': best_text, 'confidence': best_conf}]
        return []

    # --- NEW FUNCTION: Detect Violation Type ---
    def detect_violation_type(self, img):
        """
        Runs the violation.pt model to classify the scene.
        """
    ...
    def detect_and_read_plate_and_violation(self, image_bytes):
        try:
            if self.plate_model is None: return None
            ...
            # 2. Detect Violation Type (NEW)
            violation_type, violation_conf = self.detect_violation_type(img)
            
            # Calculate Fine
            fine = settings.VIOLATION_FINES.get(violation_type, settings.VIOLATION_FINES["default"])
            ...
            return {
                ...
                'violation_type': violation_type,  # New
                'violation_confidence': violation_conf, # New
                'fine_amount': fine, # New
    ...
    # ... (Keep find_vehicle_owner exactly as before) ...
    async def find_vehicle_owner(self, plate_number):
        ...
        try:
            ...
            vehicle = await mongodb.db.db.vehicles.find_one({"plateNo": {"$regex": f"^{search_key}$", "$options": "i"}})
            if not vehicle:
                relaxed = ".*".join(list(search_key))
                vehicle = await mongodb.db.db.vehicles.find_one({"plateNo": {"$regex": f"^{relaxed}$", "$options": "i"}})
    ...
    # --- UPDATED: Process Complete Detection ---
    async def process_complete_detection(self, image_bytes, location=None, camera_id=None):
        ...
        if owner_info and viol_id:
            notified = await self.send_notification(
                ...
                result['plate_number'], 
                viol_id,
                result['violation_type'], # Pass violation Type
                result['fine_amount'],    # Pass Fine
    ...
    async def save_violation(self, plate_number, confidence, ocr_confidence,
        ...
        try:
            ...
            violation = violation_doc(
                ...
                # New fields passed to document creator
