...
async def admin_create_user(name: str, email: str, role: str, password: str, location: Location, contact_number: str = None) -> str:
    ph = hash_password(password)
    return await create_user(name, email, role, ph, location.model_dump(), contact_number)

async def admin_update_user(user_id: str, fields: Dict[str, Any]) -> None:
    patch: Dict[str, Any] = {}
    for k in ("name", "email", "role", "location", "contact_number"):
